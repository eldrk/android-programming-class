...
import com.example.student.multimemo.db.MemoDatabase;
...
public class MemoInsertActivity extends AppCompatActivity {
    ...
    private void saveInput() {
    	...
    	if (photoFilename != null) {
			//사진 테이블에 추가된 레코드의 ID값을 확인하기 위한 SQL정의
    ...
    //데이터베이스에 저장된 메모 데이터를 다시 리스트뷰에 로딩
	protected void onActivityResult(int requestCode, int resultCode, Intent intent) {
		super.onActivityResult(requestCode, resultCode, intent);

		switch(requestCode) {
			//사진 촬영 액티비티로 부터의 응답처리
			case BasicInfo.REQ_PHOTO_CAPTURE_ACTIVITY:
				...
				if (resultCode == RESULT_OK) {
			    	...
			    	if (isPhotoExists) {
						...
						//사진 촬영을 통해 저장된 파일을 디코딩하여 비트맵이미지로 생성
